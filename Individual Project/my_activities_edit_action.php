...
if (isset($_FILES["fileToUpload"]) && empty($_FILES["fileToUpload"]["name"])) {
    ...
    if (!empty($id)) {
        ...
        $getCurrentImagePathQuery = "SELECT img_path FROM activities WHERE ac_id=$id";
        $result = mysqli_query($conn, $getCurrentImagePathQuery);
        ...
        if ($result && $row = mysqli_fetch_assoc($result)) {
            ...
            $sql = "UPDATE activities SET sem=$sem, year='$year', activities='$activities', position='$position', img_path='' WHERE ac_id=$id";
    ...
    $uploadOk = 0;
        ...
        if ($uploadOk) {
            ...
            if ($result && $row = mysqli_fetch_assoc($result)) {
                ...
                $sql = "UPDATE activities SET sem=$sem, year='$year', activities='$activities', position='$position', img_path='$uploadfileName' WHERE ac_id=$id";
                $status = update_DBTable($conn, $sql);
